 ...
 */
public class Uploadfile_rawmat {
    ...
    public String Uploadfile_Excel_To_DataBase(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        token = "CGC_RAW_ERP_FILE_" + new SimpleDateFormat("ddMMyy_hhmmssS").format(new Date());
    ...
    private void Insert_d_adjust_product(Connection con, PreparedStatement p, ResultSet rs, int start, int row_page, String doc_type) throws Exception {
        String Str_doc_id,
                ...
                SQL_Insert_header = "insert into d_adjust_rawmat_header(doc_id,doc_date,doc_time,doc_type,create_date,update_date,create_by,remark,update_by) values(?,?,?,?,?,?,?,?,?)",
                SQL_Insert_detail = "insert into d_adjust_rawmat_detail(doc_id,doc_date,line_no,product_id,wh_in,location_id,weight,doc_type,create_date,update_date,create_by,remark,update_by) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                p1 = con.prepareStatement("update  d_adjust_rawmat_header set remark = '" + rs.getString("remark") + "' where doc_id = '" + Str_doc_id + "'");
                p1.executeUpdate();
