...
import com.cgc.bean.DataBean_burn_out_and_split_3;
 ...
 * @author Beck
 */
public class PrintBurn_out_and_split3 {
    //static DataBean_burn_out_and_split_3 bean = new DataBean_burn_out_and_split_3();
    ...
    public void ReportBurn_out_and_split(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        ArrayList<DataBean_burn_out_and_split_3> AL_Print = new ArrayList<DataBean_burn_out_and_split_3>();
        try {
    ...
    public void Write(ArrayList<DataBean_burn_out_and_split_3> AL_Print, HttpServletResponse response,String Title) throws IOException {
        ...
        try{
        pw.println("EXP Format 003");
        pw.println("รหัส,ถ่านออกจากเตาN,ถ่านออกจากเตาS,ถ่านออกจากเตาA,N +8,S +8,A +8,N 8x30,S 8x30,A 8x30,N -30,S -30,A -30,N ฝุ่น,S ฝุ่น,A ฝุ่น,N หิน,S หิน,A หิน,Nยอดสูญเสีย,Sยอดสูญเสีย,Aยอดสูญเสีย");
        for (int i = 0; i < AL_Print.size(); i++) {
            ...
            str += objuti.NotNull(AL_Print.get(i).getCarbon_Plus8_N()) + ",";            
            str += objuti.NotNull(AL_Print.get(i).getCarbon_Plus8_S()) + ",";
            str += objuti.NotNull(AL_Print.get(i).getCarbon_Plus8_A()) + ",";            
        ...
        pw.println("ยอดรวม,ถ่านออกจากเตา,กะN,กะS,กะA,-30,ฝุ่น,หิน,ยอดสูญเสีย,'+8,8x30,");
    ...
    public ArrayList<DataBean_burn_out_and_split_3> Select_Data_burn_out_and_split(Connection con, ArrayList<DataBean_burn_out_and_split_3> AL_Obj, String str_doc_id,String Burn_no,String C_type)throws Exception {
        UtiDatabase objuti = new UtiDatabase();
        ...
        String SQL_header = "select doc_id,weight_split,lost_weight from vd_carbon_burn_out_header_check where delete_flag = 'N' and size_check = '+8' and (doc_id = '"+doc_id_A+"' or doc_id = '"+doc_id_N+"' or doc_id = '"+doc_id_S+"') and doc_id LIKE '%P%' ";
        System.out.println("Header : "+ SQL_header);
        ResultSet rs1 = null;        
        String Doc_id_check = "";        
        String Doc_id_value = "";     
        String SQL_detail = "";
        try {
            ...
            DataBean_burn_out_and_split_3 bean_header = new  DataBean_burn_out_and_split_3();
            while(rs1.next()){
            ...
            while(rs.next()){
                ...
                if(rs.getString("doc_id").indexOf("A") != -1){
                    if(rs.getString("size").equals("+8")){
                        bean_header.setCarbon_Plus8_A(rs.getString("weight"));     
                    ...
                    if(rs.getString("size").equals("+8")){
                        bean_header.setCarbon_Plus8_N(rs.getString("weight"));  
                    }else if(rs.getString("size").equals("8x30")){
                    ...
                    if(rs.getString("size").equals("+8")){
                        bean_header.setCarbon_Plus8_S(rs.getString("weight"));
                    }else if(rs.getString("size").equals("8x30")){
            ...
            if(AL_Obj.isEmpty()){
                for(int i = 1 ; i<= 2;i++){
                    DataBean_burn_out_and_split_3 Mem_AL = new DataBean_burn_out_and_split_3();
                    if(i==1){
                        ...
                        ///------------------------A----------------------------
                       Mem_AL.setCarbon_Plus8_A(bean_header.getCarbon_Plus8_A());   
                       ...
                       //-------------------------N------------------------------
                       Mem_AL.setCarbon_Plus8_N(bean_header.getCarbon_Plus8_N());   
                       ...
                       //-------------------------S------------------------------
                       Mem_AL.setCarbon_Plus8_S(bean_header.getCarbon_Plus8_S());
