...
public class PrintCSV_CARBON_OUT {
    ...
    public void ExportCSV(HttpServletRequest request, HttpServletResponse response) throws IOException, Exception {
        ...
        if (request.getParameter("export_id").equals("EXP009")) {
            SQL = " SELECT year,month,burn_no,size,ctc_i2,sum(to_number(weight,'999999999.99')) as sum_weight ";
            SQL += " FROM vd_carbon_burn_out_detail_split_size ";
            SQL += " WHERE year = '" + doc_year + "' AND delete_flag <> 'Y' ";
            SQL += " GROUP BY year,month,burn_no,size,ctc_i2 ";
            SQL += " ORDER BY year,month,burn_no,size,ctc_i2 ";
        ...
        try {
            ...
            try (PrintWriter pw = response.getWriter()) {
                ...
                while (rs.next()) {
                    ...
                    Str_Writer += objuti.NotNull(rs.getString("size")) + ",";
                    Str_Writer += objuti.NotNull(rs.getString("ctc_i2")) + ",";
                    Str_Writer += objuti.NotNull(rs.getString("burn_no")) + ",";
