 ...
 */
public class PrintReport_Rawmat_receive_balance_1 {
    ...
    public String ShowDetail(String month_id, String price_year, String location_id) throws Exception {
        ...
        String SQL_Del = "delete from tmp_rawmatt_summary_receive ; ALTER SEQUENCE seq_tmp_rawmatt_summary_receive RESTART WITH 1;";
        ...
        //String condition = Cond0 + Cond1 + " and (process_id = 'PR_022' or process_id = 'PR_023') ";
        String condition = "";
        ...
        try {
            ...
            for (int x = 1; x <= 3; x = x + 1) {
                ...
                if (x == 1) {
                    if (Integer.parseInt(month_id) == 1) {
                        condition = Cond0 + Cond1 + " and (process_id = 'PR_022') and pcat_id = 'RAW' ";
                    } else {
                        condition = Cond0 + Cond1 + " and (process_id = 'PR_022' or process_id = 'PR_117') and pcat_id = 'RAW' ";
                    }
                } else if (x == 2) {
                    condition = Cond0 + Cond1 + " and (process_id = 'PR_023') and pcat_id = 'RAW' ";
                } else if (x == 3) {
                    Cond0 = "Where (month BETWEEN '01' and '" + prev_month + "') and year = '" + price_year + "' ";
                    if (Integer.parseInt(month_id) == 1) {
                        condition = Cond0 + Cond1 + " and (process_id = 'PR_117') and pcat_id = 'RAW' ";
                    } else {
                        condition = Cond0 + Cond1 + " and (process_id = 'PR_022' or process_id = 'PR_117' or process_id = 'PR_023') and pcat_id = 'RAW' ";
                    }
                ...
                System.out.println("X = " + x + " SQL = " + SQL);
                String sql1
                ...
                if (objuti.numRowdatabase(sql1) >= 1) {
                    ...
                    while (rs.next()) {
                        ...
                        String SQL_Check_data = "SELECT count(*) as num FROM tmp_rawmatt_summary_receive "
            ...
            //To Display DATA            
            String SQL = "Select * from vtmp_rawmatt_summary_receive Order By product_id,wh_id,location_id ";

            System.out.println("SQL = " + SQL);
            String sql1 = "Select COUNT(*) as num From vtmp_rawmatt_summary_receive ";
            ...
            if (objuti.numRowdatabase(sql1) >= 1) {
                ...
                Return_String.append(("<td class=\"row5\" width=\"6%\">รับเข้า</td>\n"));
                Return_String.append(("<td class=\"row5\" width=\"6%\">เบิกเพื่อสี</td>\n"));
    ...
    public void insert_rawmatt(PrintReport_Rawmat_bigbag_balance DataBean, int x) throws Exception {
        ...
        int i = 1;
        try {
            //String SQL_Insert = "INSERT INTO tmp_rawmatt_summary_receive(product_id, wh_id,location_id,rawmatt_transfer,rawmatt_receive,rawmatt_withdraw,rawmatt_balance,create_date,create_by,update_date) values(?,?,?,?,?,?,?,?,?,?)";
            String SQL_Insert = "INSERT INTO tmp_rawmatt_summary_receive(product_id, wh_id,location_id,rawmatt_transfer,rawmatt_receive,rawmatt_withdraw,rawmatt_balance,month,year) values(?,?,?,?,?,?,?,?,?)";
            p = con.prepareStatement(SQL_Insert);
            p.setString(i, DataBean.getProduct_id());
            i += 1;
            //p.setString(i, DataBean.getWh_id());
            p.setString(i, "RAW_WH");
            i += 1;
    ...
    public void update_rawmatt(PrintReport_Rawmat_bigbag_balance DataBean, int x) throws Exception {
        ...
        try {
            //String SQL_Update = " UPDATE tmp_rawmatt_summary_receive SET " + sfield + "=?,update_by=?,update_date=? WHERE product_id=? and wh_id=? and location_id =? ";
            String SQL_Update = " UPDATE tmp_rawmatt_summary_receive SET " + sfield + "=?,update_by=?,update_date=? WHERE product_id=? and location_id =? ";
