 ...
 * @author Beck
 */
public class D_carbon_return_detail_whDAO {
private int Returnvalue;
    ...
    public int insert(DataBeanD_carbon_return_detail dataBean) throws Exception {
        ...
        String sql = "insert into d_carbon_return_detail_wh (doc_id,line_no,prd_no,size,c_type,iod,comp_palate,late_no,machine,size_cont,bag,s_weight,weight,reason_ret,remark,create_date,create_by) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(DataBeanD_carbon_return_detail dataBean) throws Exception {
        ...
        String sql =
                "update d_carbon_return_detail_wh set  prd_no=?,size=?,c_type=?,iod=?,comp_palate=?,late_no=?,machine=?,size_cont=?,bag=?,s_weight=?,weight=?,reason_ret=?,remark=?,update_date=?,update_by=? where doc_id=? and line_no=? and delete_flag = 'N' and complete_flag = 'N'";
        PreparedStatement p = null;
        try {
            p = con.prepareStatement(sql);
    ...
    public DataBeanD_carbon_return_detail ReturnEdit(String Doc_id, String Line_no, DataBeanD_carbon_return_detail bean) throws Exception {
        ...
        String SQL = "Select doc_id,doc_date,line_no,prd_no,size,c_type,iod,comp_palate,late_no,machine,size_cont,bag,s_weight,weight,reason_ret,remark,"
                + "(select vd_job_order_header.cust_name from vd_job_order_header where vd_job_order_header.doc_id = d_carbon_return_detail_wh.prd_no) as prd_name,"
                + "(select mproduct_cat.name_t from mproduct_cat where mproduct_cat.pcat_id = d_carbon_return_detail_wh.c_type) as c_type_name "
                + " from d_carbon_return_detail_wh where doc_id = '" + Doc_id + "' and line_no ='" + Line_no + "' and complete_flag ='N' and delete_flag = 'N'";
        ...
        try {
            if (inputarry.length() != 0) {
                ...
                for (int i = 0; i < strarry.length; i++) {
                    ...
                    sql = "update d_carbon_return_detail_wh set delete_flag=?,delete_date=?,delete_by=? where doc_id=? and line_no=? and delete_flag = 'N' and complete_flag = 'N'";
                    p = con.prepareStatement(sql);
                    p.setString(1, "Y");
                ...
                pclose = true;
                if (strarry.length != -1) {
                    sql = "select count(doc_id) as num from d_carbon_return_detail_wh where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag = 'N'";
                    if (objuti.numRowdatabase(sql) != 0) {
                        p = null;
                        sql2 = "Select line_no from  d_carbon_return_detail_wh where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag ='N' order by runno ";
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
                            sql = "update d_carbon_return_detail_wh set line_no=? where doc_id=? and line_no=?";
                            p = con.prepareStatement(sql);
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        SQL1 = "Select Count(doc_id) As num from  vd_carbon_return_detail_wh where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' and complete_flag ='N' ";
        ...
        SQL = "Select doc_id,line_no,prd_no,size,c_type,iod,comp_palate,late_no,machine,size_cont,bag,s_weight,weight,reason_ret,remark from vd_carbon_return_detail_wh where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "'and delete_flag = 'N' and complete_flag = 'N' order by runno";
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        ResultSet rs = null;
        ...
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='7%'><a href=\"#\"  class='link_data_transparent' onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,username','../SCREEN/CS_305.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\">" + rs.getString("prd_no") + "</a></td>\n";
    ...
    public String ShowDetailApprove(String Doc_id)throws Exception{
        ...
        SQL = "Select * from vd_carbon_return_detail_wh where doc_id ='" + objth.EncodeTexttoTIS(Doc_id) + "'and delete_flag = 'N' and complete_flag = 'N' order by runno";
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        ResultSet rs = null;
        rs = con.createStatement().executeQuery(SQL);
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='18%'><input type = 'text' id = 'product_id_"+rs.getString("line_no")+"' name = 'product_id' value = '"+ rs.getString("c_type") +"' size = 15><a href='#' ><img src='../IMAGES/BUTTON/MAIN/SEARCH20.png' alt='ค้นหา' name='IMAGE4' width='20' height='20' border='0' align='middle'  onclick=\"openBrWindow('Vproduct&textinput=product_id_"+rs.getString("line_no")+"', 31, 'Search_Detail_ProductID.jsp')\"></a></td>\n";
            strOutput += "<td class='forborder' width='12%'>" + rs.getString("size") + "</td>\n";
            strOutput += "<td class='forborder' width='9%'>" + rs.getString("bag") + "</td>\n";
            strOutput += "<td class='forborder' width='10%'>" + rs.getString("s_weight") + "</td>\n";
            strOutput += "<td class='forborder' width='10%'><input type = 'text' id = 'weight_"+rs.getString("line_no")+"' name = 'weight' value = '" + rs.getString("weight") + "' onKeyUp=\'IsNuber(this.value, this)\' size = 9><input type = 'hidden' id = 'weight_doc_id_"+rs.getString("line_no")+"' name = 'weight_doc_id' value = '"+rs.getString("weight")+"'></td>\n";
            strOutput += "<td class='forborder' width='10%'><input type = 'text' id = 'warehouse_id_"+rs.getString("line_no")+"' name = 'warehouse_id' value = '" + objuti.NotNull(rs.getString("wh_id")) + "' size = 6 readonly><a href='#' ><img src='../IMAGES/BUTTON/MAIN/SEARCH20.png' alt='ค้นหา' name='IMAGE4' width='20' height='20' border='0' align='middle'  onclick=\"openBrWindow('mwarehouse&textinput=warehouse_id_"+rs.getString("line_no")+"', 48, 'Search_Detail.jsp')\"></a></td>\n";
            strOutput += "<td class='forborder' width='7%'><input type = 'text' id = 'location_id_"+rs.getString("line_no")+"' name = 'location_id' value ='' size = 5><input type = 'hidden' id='job_id_" + rs.getString("line_no") + "' name = 'job_id' value = '" + rs.getString("prd_no") + "'></td>\n";
            strOutput += "<td class='forborder' width='8%'><div id = 'show_comfirm_"+rs.getString("line_no")+"'>รอการยืนยัน</div></td>\n";
            strOutput += "</tr>\n";
        }
        if(rs != null){
            rs.close();
