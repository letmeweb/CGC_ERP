...
public class D_carbon_friction_receive_detail_2DAO {
        ...
        try {
            if (inputarry.length() != 0) {
                ...
                for (String strarry1 : strarry) {
                    ...
                    sql = "delete from d_carbon_friction_receive_detail_2 where doc_id=? and line_no=? and delete_flag = 'N' and complete_flag = 'N'";
                    p = con.prepareStatement(sql);
                    //p.setString(1, "Y");
                ...
                if (strarry.length != -1) {
                    ...
                    if (uti.numRowdatabase(sql) != 0) {
                        sql2 = "Select runno from  d_carbon_friction_receive_detail_2 where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag ='N' order by runno ";
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
                            sql = "update d_carbon_friction_receive_detail_2 set line_no=? where doc_id=? and runno=? and delete_flag <> 'Y' ";
    ...
    public void Delete_line_no_All(DataBeanD_carbon_friction_receive_detail_2 dataBean) throws Exception {
        ...
        String sqldetail = "delete from d_carbon_friction_receive_detail_2 where doc_id=? and delete_flag = 'N' and complete_flag = 'N'";
        PreparedStatement p;
        try {
