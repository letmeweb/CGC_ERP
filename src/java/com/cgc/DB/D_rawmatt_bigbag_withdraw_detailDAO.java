...
public class D_rawmatt_bigbag_withdraw_detailDAO {
        ...
        try {
            if (inputarry.length() != 0) {
                ...
                for (String strarry1 : strarry) {
                    sql = "delete from d_rawmatt_bigbag_withdraw_detail where doc_id=? and line_no=? and complete_flag = 'N' and delete_flag = 'N'";
                    p = con.prepareStatement(sql);
                    p.setString(1, bean.getDoc_id());
                    p.setString(2, strarry1);
                    p.executeUpdate();
                }
                sql2 = "Select runno from  d_rawmatt_bigbag_withdraw_detail where doc_id ='" + bean.getDoc_id() + "' and complete_flag = 'N' and delete_flag = 'N' order by runno ";
                rs = con.createStatement().executeQuery(sql2);
                while (rs.next()) {
                    sql = "update d_rawmatt_bigbag_withdraw_detail set line_no=? where doc_id=? and runno=? and delete_flag <> 'Y' ";
    ...
    public void Delete_line_no_All(DataBeanD_rawmatt_bigbag_withdraw_detail dataBean) throws Exception {
        ...
        String sqldetail = "delete from d_rawmatt_bigbag_withdraw_detail where doc_id=? and complete_flag = 'N' and delete_flag = 'N'";
        System.out.println("Delete = " + sqldetail);
        PreparedStatement p;
        try {
