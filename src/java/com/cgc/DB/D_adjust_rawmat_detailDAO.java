 ...
 */
public class D_adjust_rawmat_detailDAO {
    ...
    public int insert(DataBeanD_adjust_product_detail dataBean) throws Exception {
        ...
        try (Connection con = objcon.openNewConnection()) {
            String sql = "insert into d_adjust_rawmat_detail (doc_id,doc_type,doc_date,line_no,product_id,location_id,weight,create_date,create_by,wh_in,wh_weight_final,wh_product_id,wh_warehouse_id,wh_location_id) "
                    + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            PreparedStatement p;
            try {
                ...
                p.setString(13, dataBean.getWh_in());
                p.setString(14, dataBean.getLocation_id());
                //Returnvalue = p.executeUpdate();

                String SqlCount = "Select Count(doc_id) As num from  d_adjust_rawmat_detail where doc_id = '" + dataBean.getDoc_id() + "' and delete_flag = 'N' and line_no = '" + dataBean.getLine_no() + "' ;";
    ...
    public int update(DataBeanD_adjust_product_detail dataBean) throws Exception {
        ...
        try (Connection con = dbConnect.openNewConnection()) {
            String sql
                    = "update d_adjust_rawmat_detail set  product_id=?,location_id=?,weight=?,update_date=?,update_by=?,wh_in=?"
                    + ",wh_weight_final=?,wh_product_id=?,wh_warehouse_id=?,wh_location_id=? "
            ...
            try {
                ...
                p.setString(10, dataBean.getLocation_id());
    ...
    public DataBeanD_adjust_product_detail ReturnEdit(String Doc_id, String Line_no, DataBeanD_adjust_product_detail bean) throws Exception {
        DBConnect dbConnect = new DBConnect();
        try (Connection con = dbConnect.openNewConnection()) {
            String SQL = "select d_adjust_rawmat_detail.runno,d_adjust_rawmat_detail.doc_id,d_adjust_rawmat_detail.line_no,d_adjust_rawmat_detail.product_id,d_adjust_rawmat_detail.wh_in,"
                    + "(select vproduct.name_t from vproduct where vproduct.product_id = d_adjust_rawmat_detail.product_id) as prod_name,"
                    + "(select vproduct.pgroup_name from vproduct where vproduct.product_id = d_adjust_rawmat_detail.product_id) as pgroup_name,"
                    + "(select vproduct.pcat_name from vproduct where vproduct.product_id = d_adjust_rawmat_detail.product_id) as  pcat_name,"
                    + "(select mwarehouse.name_t from mwarehouse where mwarehouse.warehouse_id = d_adjust_rawmat_detail.wh_in) as warehouse_name,"
                    + "d_adjust_rawmat_detail.location_id,d_adjust_rawmat_detail.weight,"
                    + "d_adjust_rawmat_detail.delete_flag,d_adjust_rawmat_detail.cancel_flag,d_adjust_rawmat_detail.complete_flag,d_adjust_rawmat_detail.retro_flag"
                    + " from d_adjust_rawmat_detail  where doc_id = '" + Doc_id + "' and line_no ='" + Line_no + "' and delete_flag = 'N' ";
            //+ "and complete_flag = 'N' ";
            ...
            try {
                if (inputarry.length() != 0) {
                    ...
                    for (String strarry1 : strarry) {
                        sql = "update d_adjust_rawmat_detail set delete_flag=?,delete_date=?,delete_by=? where doc_id=? and line_no=? and delete_flag = 'N' and complete_flag = 'N'";
                        p = con.prepareStatement(sql);
                        p.setString(1, "Y");
                    ...
                    if (strarry.length != -1) {
                        sql = "select count(doc_id) as num from d_adjust_rawmat_detail where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag = 'N'";
                        if (uti.numRowdatabase(sql) != 0) {
                            //p = null;
                            sql2 = "Select line_no from  d_adjust_rawmat_detail where doc_id ='" + bean.getDoc_id() + "' and delete_flag = 'N' and complete_flag ='N' order by runno ";
                            rs = con.createStatement().executeQuery(sql2);
                            while (rs.next()) {
                                sql = "update d_adjust_rawmat_detail set line_no=? where doc_id=? and line_no=?";
                                p = con.prepareStatement(sql);
    ...
    public void Delete_line_no_All(DataBeanD_adjust_product_detail dataBean) throws Exception {
        ...
        DBConnect dbConnect = new DBConnect();
        try (Connection con = dbConnect.openNewConnection()) {
            String sqldetail = "update d_adjust_rawmat_detail set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag = 'N' and complete_flag = 'N'";
            PreparedStatement p;
            try {
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        SQL1 = "Select Count(doc_id) As num from  d_adjust_rawmat_detail where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' and complete_flag ='N' ";
        int a = uti.numRowdatabase(SQL1);
        strOutput += "<table width='100%' align='center' border='0' cellpadding='0' cellspacing='1' class='inner'>";
        SQL = "select d_adjust_rawmat_detail.runno,d_adjust_rawmat_detail.doc_id,d_adjust_rawmat_detail.line_no,d_adjust_rawmat_detail.product_id,d_adjust_rawmat_detail.wh_in,"
                + "(select vproduct.name_t from vproduct where vproduct.product_id = d_adjust_rawmat_detail.product_id) as prod_name,"
                + "(select vproduct.pgroup_name from vproduct where vproduct.product_id = d_adjust_rawmat_detail.product_id) as pgroup_name,"
                + "(select mwarehouse.name_t from mwarehouse where mwarehouse.warehouse_id = d_adjust_rawmat_detail.wh_in) as warehouse_name ,d_adjust_rawmat_detail.location_id,d_adjust_rawmat_detail.weight,"
                + "d_adjust_rawmat_detail.delete_flag,d_adjust_rawmat_detail.cancel_flag,d_adjust_rawmat_detail.complete_flag,d_adjust_rawmat_detail.retro_flag"
                + " from d_adjust_rawmat_detail  where doc_id = '" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag = 'N' "
                //+ "and complete_flag = 'N' "
                + "order by runno";
        DBConnect objcon = new DBConnect();
        try (Connection con = objcon.openNewConnection()) {
            ...
            while (rs.next()) {
                ...
                strOutput += "<td class='forborder' width='7%'><a href=\"#\"  class='link_data_transparent' onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",username,doc_id','../SCREEN/CS_117_1.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("line_no") + "</a></td>";
    ...
    public void Update_Detail_Data(String doc_id) throws Exception {
        ...
        sql = " select sum(to_number(weight,'999999.99')) as total_weight from d_adjust_rawmat_detail "
        ...
        try (Connection con = objcon.openNewConnection()) {
            ...
            String sql_header = " update d_adjust_rawmat_header set total_weight_adjust= " + total_weight
