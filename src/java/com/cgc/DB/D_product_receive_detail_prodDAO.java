 ...
 */
public class D_product_receive_detail_prodDAO {
    ...
    public int insert(DataBeanD_product_receive_detail_prod dataBean) throws Exception {
        ...
        String sql = "insert into d_product_receive_detail_prod "
                + "(doc_id,doc_date,line_no,wh_in,wh_out,machine,shift,size,weight,palate_complete_code,palate_no,palate_complete,ctc_i2,bag,emp_id,create_date,create_by,palate_chk,unit_id,job_type_id,job_type_desc,size_by,process_order) "
                + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        PreparedStatement p;
        try {
            ...
            p.setString(22, dataBean.getSize_by());
            p.setString(23, dataBean.getProcess_order());

            String SqlCount = "Select Count(doc_id) As num from  d_product_receive_detail_prod where doc_id = '" + dataBean.getDoc_id() + "' and delete_flag = 'N' and line_no = '" + dataBean.getLine_no() + "' ;";
    ...
    public int update(DataBeanD_product_receive_detail_prod dataBean) throws Exception {
        ...
        String sql = "update d_product_receive_detail_prod set  "
                + "doc_date=?,wh_in=?,wh_out=?,machine=?,shift=?,size=?,weight=?,palate_complete_code=?,palate_no=?,palate_complete=?,ctc_i2=?,bag=?,emp_id=?,update_date=?,update_by=?,palate_chk=?,unit_id=?,job_type_id=?,job_type_desc=?,size_by=?,process_order=? "
        ...
        try {
            ...
            p.setString(20, dataBean.getSize_by());
            p.setString(21, dataBean.getProcess_order());
            p.setString(22, dataBean.getDoc_id());
            p.setString(23, dataBean.getLine_no());
    ...
    public DataBeanD_product_receive_detail_prod ReturnEdit(String Doc_id, String Line_no, DataBeanD_product_receive_detail_prod bean) throws Exception {
        ...
        String SQL = "Select doc_id,doc_date,wh_in,wh_out,line_no,machine,shift,size,weight,palate_complete_code,palate_no,palate_complete,ctc_i2,bag,emp_id,"
                + "(select vemployee_fullname.fullname from vemployee_fullname where vemployee_fullname.emp_id = d_product_receive_detail_prod.emp_id) as emp_name,palate_chk,unit_id,"
                + "(select munit_code.name_t from munit_code where munit_code.unit_id = d_product_receive_detail_prod.unit_id) as unit_name,job_type_id,job_type_desc,size_by,process_order "
                + "from d_product_receive_detail_prod where doc_id = '" + Doc_id + "' and line_no ='" + Line_no + "' and delete_flag <> 'Y' and complete_flag <> 'Y'";
        ...
        try {
            while (rs.next()) {
                ...
                bean.setProcess_order(rs.getString("process_order"));
                System.out.println("size_by : " + rs.getString("size_by"));
            }
        } finally {
            try {
                rs.close();
        ...
        try {
            ...
            if (inputarry.length() != 0) {
                ...
                for (String strarry1 : strarry) {
                    sql = "update d_product_receive_detail_prod set delete_flag=?,delete_date=?,delete_by=? where doc_id=? and line_no=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
                    p = con.prepareStatement(sql);
                    p.setString(1, "Y");
                ...
                pclose = true;
                if (strarry.length != -1) {
                    sql = "select count(doc_id) as num from d_product_receive_detail_prod where doc_id ='" + bean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y'";
                    if (objuti.numRowdatabase(sql) != 0) {
                        sql2 = "Select line_no from  d_product_receive_detail_prod where doc_id ='" + bean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno ";
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
                            sql = "update d_product_receive_detail_prod set line_no=? where doc_id=? and line_no=?";
                            p = con.prepareStatement(sql);
    ...
    public void Delete_line_no_All(DataBeanD_product_receive_detail_prod dataBean) throws Exception {
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sqldetail = "update d_product_receive_detail_prod set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
        PreparedStatement p;
        try {
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        SQL1 = "Select Count(doc_id) As num from  d_product_receive_detail_prod where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' ";
        int a = objuti.numRowdatabase(SQL1);
        SQL = "Select doc_id,process_order,line_no,size,size,weight,machine,shift,palate_complete_code,palate_no,palate_complete,ctc_i2,bag,(select mjob_type.name_t from mjob_type where mjob_type.job_type_id = d_product_receive_detail_prod.job_type_id) as job_type_name,job_type_desc,size_by from d_product_receive_detail_prod where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno";
        DBConnect objcon = new DBConnect();
        try (Connection con = objcon.openNewConnection()) {
            ...
            while (rs.next()) {
                ...
                strOutput += "<td class='forborder' width='7%'><a href=\"#\"  class='link_data_transparent' onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,doc_date,username','../SCREEN/CS_045.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\">" + rs.getString("line_no") + "</a></td>";
                strOutput += "<td class='forborder' width='10%'>" + rs.getString("size") + "&nbsp;</td>";
                strOutput += "<td class='forborder' width='10%'>" + rs.getString("weight") + "&nbsp;</td>";
                strOutput += "<td class='forborder' width='10%'>" + rs.getString("job_type_name") + rs.getString("job_type_desc") + "-" + rs.getString("process_order") + "&nbsp;</td>";
    ...
    public ArrayList<DataBeanD_product_receive_detail_prod> selectDataSearch(String id) throws Exception {
        DataBeanD_product_receive_detail_prod dataBean;
        ArrayList<DataBeanD_product_receive_detail_prod> list = new ArrayList<>();
        DBConnect dbConnect = new DBConnect();
        ResultSet rs;
        Connection con = dbConnect.openNewConnection();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                dataBean.setProcess_order(rs.getString("process_order"));
