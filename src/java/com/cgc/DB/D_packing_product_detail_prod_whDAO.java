 ...
 * @author Beck
 */
public class D_packing_product_detail_prod_whDAO {
    ...
    public int insert(DataBeanD_packing_product_detail_prod dataBean) throws Exception {
        ...
        String sql = "insert into d_packing_product_detail_prod_wh "
                + "(doc_id,line_no,wh_id,product_id,size,i2,bag,weight,remark,create_date,create_by,weight_unit,product_no) "
                + "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(DataBeanD_packing_product_detail_prod dataBean) throws Exception {
        ...
        String sql = "update d_packing_product_detail_prod_wh set  "
                + "wh_id=?,product_id=?,size=?,i2=?,bag=?,weight=?,remark=?,update_date=?,update_by=?,weight_unit=?,product_no=?"
        ...
        try {
            ...
            p.setString(3, dataBean.getSize());
            p.setString(4, dataBean.getI2());
            ...
            p.setString(10, dataBean.getWeight_unit());
            p.setString(11, dataBean.getProduct_no());
    ...
    public DataBeanD_packing_product_detail_prod ReturnEdit(String Doc_id, String Line_no, DataBeanD_packing_product_detail_prod bean) throws Exception {
        ...
        String SQL = "Select doc_id,line_no,wh_id,product_id,size,i2,bag,weight,remark,weight_unit,product_no,warehouse_name,"
                + "(select m_carbon_size.name_t from m_carbon_size where m_carbon_size.size_id = to_number(vd_packing_product_detail_prod_wh.size,'99999')) as size_name,pack_repacking_status,carbon_trap_status"
                + " from vd_packing_product_detail_prod_wh "
        ...
        try {
            ...
            if (inputarry.length() != 0) {
                ...
                for (int i = 0; i < strarry.length; i++) {
                    ...
                    sql = "update d_packing_product_detail_prod_wh set delete_flag=?,delete_date=?,delete_by=? where doc_id=? and line_no=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
                    p = con.prepareStatement(sql);
                    p.setString(1, "Y");
                ...
                pclose = true;
                if (strarry.length != -1) {
                    sql = "select count(doc_id) as num from d_packing_product_detail_prod_wh where doc_id ='" + bean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y'";
                    if (objuti.numRowdatabase(sql) != 0) {
                        p = null;
                        sql2 = "Select line_no,runno from  d_packing_product_detail_prod_wh where doc_id ='" + bean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno ";
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
                            sql = "update d_packing_product_detail_prod_wh set line_no=? where doc_id=? and line_no=? and runno=?";
                            p = con.prepareStatement(sql);
                        ...
                        sql2 = "select bag,weight from d_packing_product_detail_prod_wh where doc_id ='" + bean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno ";
                        rs = null;
                        rs = con.createStatement().executeQuery(sql2);
                        while (rs.next()) {
    ...
    public String Delete_line_no_All(DataBeanD_packing_product_detail_prod dataBean) throws Exception {
        ...
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sqldetail = "update d_packing_product_detail_prod_wh set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
        PreparedStatement p = null;
        try {
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        SQL1 = "Select Count(doc_id) As num from  d_packing_product_detail_prod_wh where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' ";
        int a = objuti.numRowdatabase(SQL1);
        SQL = "Select doc_id,line_no,wh_id,product_id,size,i2,bag,weight,size_desc,weight_unit,warehouse_name,product_no,doc_type from vd_packing_product_detail_prod_wh where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno";
        DBConnect objcon = new DBConnect();
        Connection con = objcon.openNewConnection();
        ResultSet rs = null;
        ...
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='7%'><a href=\"#\"  class='link_data_transparent' onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,total_bag,total_weight,username','../SCREEN/CS_308.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\">" + rs.getString("line_no") + "</a></td>";
