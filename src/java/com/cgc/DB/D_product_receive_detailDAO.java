 ...
 */
public class D_product_receive_detailDAO {
    ...
    public int insert(DataBeanD_product_receive_detail dataBean) throws Exception {
        ...
        String sql = "insert into d_product_receive_detail "
                + "(doc_id,doc_date,line_no,wh_in,wh_out,burn_no,location_id,size,job_edit_no,ctc_i2,qty,shift,palate_no,emp_id,create_date,wh_id,create_by,unit_id,c_type,job_type_id,job_type_desc,remark,product_id,process_order)"
                + " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        PreparedStatement p;
        try {
            ...
            p.setString(19, dataBean.getC_type());
            System.out.println("C_Type ID : " + dataBean.getC_type());
            p.setString(20, dataBean.getJob_type_id());
            p.setString(21, dataBean.getJob_type_desc());
            p.setString(22, dataBean.getRemark());
            p.setString(23, dataBean.getC_type());
            p.setString(24, dataBean.getProcess_order());

            String SqlCount = "Select Count(doc_id) As num from  d_product_receive_detail where doc_id = '" + dataBean.getDoc_id() + "' and delete_flag = 'N' and line_no = '" + dataBean.getLine_no() + "' ;";
    ...
    public int update(DataBeanD_product_receive_detail dataBean) throws Exception {
        ...
        String sql
                ...
                + "set  doc_date=?,wh_in=?,wh_out=?,burn_no=?,location_id=?,size=?,job_edit_no=?,ctc_i2=?,qty=?,shift=?,palate_no=?,emp_id=?,update_date=?,wh_id=?,update_by=?,unit_id=?,c_type=?,job_type_id=?,job_type_desc=?,remark=?,product_id=?,process_order=? "
        ...
        try {
            ...
            p.setString(17, dataBean.getC_type());
            System.out.println("Update C_Type ID : " + dataBean.getC_type());
            p.setString(18, dataBean.getJob_type_id());
            p.setString(19, dataBean.getJob_type_desc());
            p.setString(20, dataBean.getRemark());
            p.setString(21, dataBean.getC_type());
            p.setString(22, dataBean.getProcess_order());
            p.setString(23, dataBean.getDoc_id());
            p.setString(24, dataBean.getLine_no());
            Returnvalue = p.executeUpdate();
    ...
    public DataBeanD_product_receive_detail ReturnEdit(String Doc_id, String Line_no, DataBeanD_product_receive_detail bean) throws Exception {
        ...
        String SQL = "Select "
                ...
                + "(select munit_code.name_t from munit_code where munit_code.unit_id = d_product_receive_detail.unit_id) as unit_name,process_order "
                + "from d_product_receive_detail where doc_id = '" + Doc_id + "' and line_no ='" + Line_no + "' and delete_flag <> 'Y' and complete_flag <> 'Y'";
        ...
        try {
            while (rs.next()) {
                ...
                System.out.println("c_type ID = " + rs.getString("c_type"));
            }
        } finally {
            try {
                rs.close();
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        SQL1 = "Select Count(doc_id) As num from  d_product_receive_detail where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' ";
        int a = uti.numRowdatabase(SQL1);
        SQL = "Select doc_id,line_no,burn_no,location_id,wh_id,size,job_edit_no,ctc_i2,qty,process_order,(select munit_code.name_t from munit_code where munit_code.unit_id=d_product_receive_detail.unit_id) as unit_name,c_type,shift,(select mjob_type.name_t from mjob_type where mjob_type.job_type_id = d_product_receive_detail.job_type_id) as job_type_name,job_type_desc from d_product_receive_detail where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno";
        DBConnect objcon = new DBConnect();
        try (Connection con = objcon.openNewConnection()) {
            ...
            while (rs.next()) {
                ...
                strOutput += "<td class='forborder' width='7%'><a href=\"#\"  class='link_data_transparent' onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,doc_date,username','../SCREEN/CS_037.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\">" + rs.getString("line_no") + "</a></td>";
                strOutput += "<td class='forborder' width='15%'>" + rs.getString("c_type") + "&nbsp;</td>";
                strOutput += "<td class='forborder' width='15%'>" + rs.getString("job_type_name") + rs.getString("job_type_desc") + "-" + rs.getString("process_order") + "&nbsp;</td>";
