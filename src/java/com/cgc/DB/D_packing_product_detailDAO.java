...
public class D_packing_product_detailDAO {
    ...
    public int insert(DataBeanD_packing_product_detail dataBean) throws Exception {
        ...
        try {
            ...
            String SqlCount = "Select Count(doc_id) As num from  d_packing_product_detail where doc_id = '" + dataBean.getDoc_id() + "' and delete_flag = 'N' and line_no = '" + dataBean.getLine_no() + "' ;";
    ...
    public int update(DataBeanD_packing_product_detail dataBean) throws Exception {
        ...
        String sql
                ...
                + "wh_id=?,product_id=?,place_prd=?,palate_no=?,size=?,i2=?,bag=?,weight=?,remark=?,update_date=?,update_by=?,fraction_bag=?,product_no=?,doc_type=?,pack_repacking_status=?  "
        ...
        try {
            ...
            p.setString(14, dataBean.getDoc_type());
            p.setString(15, dataBean.getPack_repacking_status());
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        SQL1 = "Select Count(doc_id) As num from  d_packing_product_detail where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' ";
        int a = uti.numRowdatabase(SQL1);
        SQL = "Select doc_id,line_no,warehouse_name,product_id,place_prd,palate_no,i2,bag,weight,fraction_bag,size_desc,place_desc,product_no,doc_type,pack_repacking_status from vd_packing_product_detail where doc_id ='" + objth.EncodeTexttoTIS(doc_id) + "' and delete_flag <> 'Y' and complete_flag <> 'Y' order by runno";
    ...
    public void Update_Detail_Data(String doc_id, String table_header, String table_detail) throws Exception {
        String sql;
        Double total_weight = 0.00;
        sql = " select sum(to_number(weight,'999999.99')) as total_weight from " + table_detail
        ...
        String sql_header = " update " + table_header + "  set total_weight_adjust= " + total_weight
