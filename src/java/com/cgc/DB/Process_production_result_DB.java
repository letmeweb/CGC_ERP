 ...
 */
public class Process_production_result_DB {
    ...
    public void generater_transaction_process(String job_id, String user_login, Timestamp ts) throws Exception {
        ...
        try {
            ...
            SQL_Res1 = " Select job_id,job_type_id,size,sum(to_number(weight,'999999.99')) as weight ,process_order"
                    + " from vd_product_receive_detail_prod where job_id = '" + job_id
                    + "' and job_type_id in " + job_type_id_product
                    + " Group By job_id,job_type_id,size,process_order"
                    + " Order By job_id,to_number(process_order,'99'),job_type_id,size desc";
            ...
            while (rs.next()) {
                ...
                if (rs.getInt("process_order") >= 2 && !(rs.getString("size").equals(Size_By_Job))) {
                    System.out.println("1 job_id = " + rs.getString("job_id") + " | job_type_id = "
                            + rs.getString("job_type_id") + " | " + rs.getString("size") + " | " + rs.getString("weight")+ " | " + rs.getString("process_order"));

                    int statusnum = objuti.numRowdatabase("Select Count(*) As num from  d_product_receive_detail_result"
                            + " where job_id = '" + rs.getString("job_id") + "'"
                            + " and size = '" + rs.getString("size") + "'"
                            + " and job_type_id = '" + rs.getString("job_type_id") + "'"
                            + " and delete_flag <> 'Y'");
                    if (statusnum > 0) {
                ...
                } else if (rs.getInt("process_order") <= 1) {
            ...
            SQL_Res1 = " Select job_id,job_type_id,size,sum(to_number(qty,'999999.99')) as weight from vd_product_receive_detail where job_id = '" + job_id
                    + "' and job_type_id in " + job_type_id_product
