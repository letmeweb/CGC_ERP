 ...
 */
public class Process_transaction_friction_summaryDB_New {

    public void generater_transaction_process(String Time_Loop, String date_from, String date_to, String process_id, String table, String doc_type, String r, String username) throws Exception {
        ...
        try (Connection con = new DBConnect().openNewConnection()) {
            ...
            if (Time_Loop.equalsIgnoreCase("1")) {
    ...
    private void insert(ArrayList<DataBean_Transaction_Process_wh_summary> obj_AL, Connection con, PreparedStatement p) throws Exception {
        ...
        try {
            p = con.prepareStatement("insert into tmp_stock_rawmat_friction_daily "
                    + "(stock_type,doc_date,weight_value,price_value,create_date,log_id) "
                    + "values"
                    + " (?,?,?,?,?,?)");
