 ...
 */
public class D_adjust_rawmat_headerDAO {
    ...
    public int insert(DataBeanD_adjust_product_header dataBean) throws Exception {
        ...
        try (Connection con = objcon.openNewConnection()) {
            String sql = "insert into D_adjust_rawmat_header "
            ...
            try {
                ...
                sql = "select count(doc_id) as num from d_adjust_rawmat_detail where doc_id ='" + dataBean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y'";

                if (objuti.numRowdatabase(sql) != 0) {
                    sql = "update d_adjust_rawmat_detail set doc_date=?,doc_type=?,create_by=? where doc_id = ? and delete_flag <> 'Y' and complete_flag <> 'Y'";
                    //p = null;
                    p = con.prepareStatement(sql);
    ...
    public int update(DataBeanD_adjust_product_header dataBean) throws Exception {
        ...
        try (Connection con = dbConnect.openNewConnection()) {
            String sql
                    = "update D_adjust_rawmat_header set  doc_date=?,doc_time=?,doc_type=?,remark=?,update_date=?,update_by=?,complete_flag=? where doc_id=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
            PreparedStatement p ;
            try {
                p = con.prepareStatement(sql);
                p.setString(1, dataBean.getDoc_date());
                ...
                sql = "select count(doc_id) as num from d_adjust_rawmat_detail where doc_id ='" + dataBean.getDoc_id() + "' and delete_flag <> 'Y' and complete_flag <> 'Y'";
                if (objuti.numRowdatabase(sql) != 0) {
                    sql = "update d_adjust_rawmat_detail set doc_date=?,doc_type=?,update_by=?,complete_flag=? where doc_id = ? and delete_flag <> 'Y' and complete_flag <> 'Y'";
                    //p = null;
                    p = con.prepareStatement(sql);
                    p.setString(1, dataBean.getDoc_date());
                    p.setString(2, dataBean.getDoc_type());
    ...
    public void UpDel(DataBeanD_adjust_product_header dataBean) throws Exception {
        ...
        DBConnect dbConnect = new DBConnect();
        try (Connection con = dbConnect.openNewConnection()) {
            String sqlheader = "update D_adjust_rawmat_header set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
            String sqldetail = "update d_adjust_rawmat_detail set delete_flag = ?,delete_date=?,delete_by=? where doc_id=? and delete_flag <> 'Y' and complete_flag <> 'Y'";
            PreparedStatement p;
            try {
                for (int i = 0; i < 2; i++) {
                    if (i == 0) {
    ...
    public void update_sum_header(DataBeanD_adjust_product_header dataBean) throws Exception {
        ...
        try (Connection con = objcon.openNewConnection()) {
            ...
            String sql = "select sum(to_number(weight,'9999999.99')) as weight from d_adjust_rawmat_detail where doc_id = '" + dataBean.getDoc_id() + "'";
            ...
            String sql_update = "update d_adjust_rawmat_header set total_weight_adjust=? where doc_id = ? and delete_flag <> 'Y' ";
