...
<%!    UtiDatabase uti = new UtiDatabase();
    ...
    D_carbon_analyze_detailDAO objex = new D_carbon_analyze_detailDAO();
...
<%
        int statusck = 0;
            ...
            if (request.getParameter("status_a") != null) {
                statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_carbon_analyze_detail where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'Y'");
                statusck = 1;
                objdb.setLine_no(Integer.toString(statusnum + 1));

            } else if (request.getParameter("status_u") != null) {
                statusck = 2;
                objdb.setLine_no(request.getParameter("header_id"));
