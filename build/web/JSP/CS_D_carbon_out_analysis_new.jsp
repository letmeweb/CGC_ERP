    ...
    if (statusck == 1 || statusck == 2) {
        ...
        if (maxline != 0) {
            for (int count = 1; count <= maxline; count++) {
                ...
                bean.setBu1((request.getParameter("bu1_"+count)).trim().equals("")||(request.getParameter("bu1_"+count)).trim().equals("-")?"0":en.EncodeTexttoTIS(request.getParameter("bu1_"+count)).trim());
                bean.setAvg1((request.getParameter("avg1_"+count)).trim().equals("")||(request.getParameter("avg1_"+count)).trim().equals("-")?"0":en.EncodeTexttoTIS(request.getParameter("avg1_"+count)).trim());                
                bean.setC_type2((request.getParameter("c_type2_"+count)).trim().equals("")?"":(request.getParameter("c_type2_"+count)).trim());
                bean.setBu2((request.getParameter("bu2_"+count)).trim().equals("")||(request.getParameter("bu2_"+count)).trim().equals("-")?"0":(request.getParameter("bu2_"+count)).trim());
                bean.setAvg2((request.getParameter("avg2_"+count)).trim().equals("")||(request.getParameter("avg2_"+count)).trim().equals("-")?"0":en.EncodeTexttoTIS(request.getParameter("avg2_"+count)).trim());
                bean.setC_type3((request.getParameter("c_type3_"+count)).trim().equals("")?"":(request.getParameter("c_type3_"+count)).trim());
                bean.setBu3((request.getParameter("bu3_"+count)).trim().equals("")||(request.getParameter("bu3_"+count)).trim().equals("-")?"0":(request.getParameter("bu3_"+count)).trim());
                bean.setAvg3((request.getParameter("avg3_"+count)).trim().equals("")||(request.getParameter("avg3_"+count)).trim().equals("-")?"0":en.EncodeTexttoTIS(request.getParameter("avg3_"+count)).trim());
                objDetail.add(bean);
            }
        }
    ...
    switch (statusck) {
        ...
        case 2:
            if (request.getParameter("confirm") != null && request.getParameter("confirm").equals("true")) {
                objex.update_new(objdb, objDetail);
