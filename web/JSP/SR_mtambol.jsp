...
<body>
    <form name="serForm" method="get" action="SR_mtambol.jsp">
                ...
                <tr >
                    ...
                    <td  colspan="2" class="fcentercenter">
                        <table cellpadding="0" cellspacing="0" >
                            ...
                            <tr>
                                <td colspan="3" align="left"><div align="right">ชื่อตำบล :</div></td>
                            ...
                            <tr>
                                <td colspan="2" class="header2"><div align="left">รหัสตำบล</div></td><td colspan="2" class="header2"><div align="left">ชื่อตำบล</div></td>
                            </tr>
<%
if(searchtxt == null)
{
String sql2="select * from mtambol order by name_t limit "+row_page+" offset "+start;
...
%>
                        <tr>
                            <td colspan="2" align="center"><a  href="#" onClick="gettoShow('<%=getshowdata%>','tambol_code,tambol_name')"><%=rs2.getString("tambol_code")%></a></td>
...
if(screen>1)
{%>
                          <a href="../JSP/SR_mtambol.jsp?screen=<%=screen-1%>">ย้อนกลับ</a>
                          <%
}
...
{
%>
	| <a href="../JSP/SR_mtambol.jsp?screen=<%=i%>" ><%=i%></a>|
    <%
}
...
for(int y=(screen-5);y<=(screen+5)&& y<= total_page ;y++)
{
	...
	{
	%>
		| <a href="../JSP/SR_mtambol.jsp?screen=<%=y%>" ><%=y%></a>|
	    <%
	}
}
}
...
{%>
        <a href="../JSP/SR_mtambol.jsp?screen=<%=screen+1%>">ไปข้างหน้า</a>
        <%
}
else{;
...
{
	String sql2="select * from mtambol  where name_t like '"+searchtxt+"%' limit "+row_page+" offset "+start;
	ResultSet rs2 = Conn.createStatement().executeQuery(sql2);
	while(rs2.next())
	{
		count++;
		String getshowdata = rs2.getString("tambol_code")+","+ rs2.getString("name_t");
	%>
                      </div>
<tr>
	                            <td colspan="2" align="center"><a  href="#" onClick="gettoShow('<%=getshowdata%>','tambol_code,tambol_name')"><%=rs2.getString("tambol_code")%></a></td>
	...
	if(screen>1)
	{%>
	<a href="../JSP/SR_mtambol.jsp?screen=<%=screen-1%>&searchtxt=<%= searchtxt%>">ย้อนกลับ</a>
	<%
	}
	if((screen%10)!= 0 && screen <= 10)
	{
	...
	{
	%>
		| <a href="../JSP/SR_mtambol.jsp?screen=<%=i%>&searchtxt=<%= searchtxt%>" ><%=i%></a>|
	<%
	}
	...
	for(int y=(screen-5);y<=(screen+5)&& y<= total_page ;y++)
	{
		...
		{
		%>
			| <a href="../JSP/SR_mtambol.jsp?screen=<%=y%>&searchtxt=<%= searchtxt%>" ><%=y%></a>|
		<%
		}
	}
	}
	if(screen<total_page)
	{%>
	<a href="../JSP/SR_mtambol.jsp?screen=<%=screen+1%>&searchtxt=<%= searchtxt%>">ไปข้างหน้า</a>
	<%
	}
	else{;
