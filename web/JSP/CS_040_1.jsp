...
<%@page import="com.cgc.DB.D_packing_product_headerDAO_40_1"%>
...
<%!    ThaiUtil en = new ThaiUtil();
    ...
    D_packing_product_headerDAO_40_1 objex = new D_packing_product_headerDAO_40_1();
...
<%
            ...
            if (request.getParameter("status_d") == null) {
                //statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_packing_product_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'N'");
                  statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_packing_product_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'N' and count_send_complete <> '0' ");                
                if (statusnum == 0) {
                    ...
                    if (uti.numRowdatabase("Select Count(doc_id) As num from  d_packing_product_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'N'") == 1) {
                        statusck = -1;
                    } else {
                        statusck = 1;
                    }
