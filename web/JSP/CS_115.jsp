<%@page import="com.cgc.DB.D_adjust_product_headerDAO"%>
...
<%!    UtiDatabase uti = new UtiDatabase();
    ...
    D_adjust_product_headerDAO objex = new D_adjust_product_headerDAO();
    Timestamp ts;
    String runer_id;    
...
<%
            ...
            if (request.getParameter("status_d") == null && request.getParameter("runer_id") == null) {
                statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_adjust_product_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'Y'");
                if (statusnum == 0) {
                    if (uti.numRowdatabase("Select Count(doc_id) As num from  d_adjust_product_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'Y'") == 1) {
                        statusck = -1;
                    } else {
                        statusck = 1;
                    }
            ...
            switch (statusck) {
                ...
                case 4:
                    ...
                    out.print(objrun.Run_Next_id(runer_id,"d_adjust_product_header", "d_adjust_product_detail"));
                    break;
                default:
